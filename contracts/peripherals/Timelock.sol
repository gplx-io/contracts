...
contract Timelock is ITimelock {
    ...
    function initGplpManager() external onlyAdmin {
        IGplpManager _gplpManager = IGplpManager(gplpManager);

        IMintable gplp = IMintable(_gplpManager.gplp());
        gplp.setMinter(gplpManager, true);

        IUSDG usdg = IUSDG(_gplpManager.usdg());
        usdg.addVault(gplpManager);

        IVault vault = _gplpManager.vault();
        vault.setManager(gplpManager, true);
    }

    function initRewardRouter() external onlyAdmin {
        IRewardRouterV2 _rewardRouter = IRewardRouterV2(rewardRouter);

        IHandlerTarget(_rewardRouter.feeGplpTracker()).setHandler(rewardRouter, true);
        IHandlerTarget(_rewardRouter.stakedGplpTracker()).setHandler(rewardRouter, true);
        IHandlerTarget(gplpManager).setHandler(rewardRouter, true);
    }
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external onlyAdmin {
        IGplpManager(gplpManager).setShortsTrackerAveragePriceWeight(_shortsTrackerAveragePriceWeight);
    }

    function setGplpCooldownDuration(uint256 _cooldownDuration) external onlyAdmin {
        require(_cooldownDuration < 2 hours, "Timelock: invalid _cooldownDuration");
        IGplpManager(gplpManager).setCooldownDuration(_cooldownDuration);
    }
