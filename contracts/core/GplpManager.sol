...
contract GplpManager is ReentrancyGuard, Governable, IGplpManager {
    ...
    uint256 public constant GPLP_PRECISION = 10 ** 18;
    ...
    constructor(address _vault, address _usdg, address _gplp, address _shortsTracker, uint256 _cooldownDuration) public {
        gov = msg.sender;
        vault = IVault(_vault);
        usdg = _usdg;
        gplp = _gplp;
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external override onlyGov {
        require(shortsTrackerAveragePriceWeight <= BASIS_POINTS_DIVISOR, "GplpManager: invalid weight");
    ...
    function getPrice(bool _maximise) external view returns (uint256) {
        ...
        uint256 supply = IERC20(gplp).totalSupply();
        return aum.mul(GPLP_PRECISION).div(supply);
    }
