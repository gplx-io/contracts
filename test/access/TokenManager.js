...
describe("TokenManager", function () {
  ...
  beforeEach(async () => {
    ...
    gplxTimelock = await deployContract("GplxTimelock", [
      wallet.address,
      5 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
      user0.address,
  ...
  it("approve", async () => {
    ...
    await expect(tokenManager.connect(wallet).approve(gplx.address, user2.address, expandDecimals(5, 18), 1))
      .to.be.revertedWith("TokenManager: action not signalled")
  ...
  it("signalSetGov", async () => {
    await expect(tokenManager.connect(user0).signalSetGov(timelock.address, gplx.address, user1.address))
      .to.be.revertedWith("TokenManager: forbidden")

    await tokenManager.connect(wallet).signalSetGov(timelock.address, gplx.address, user1.address)
  })

  it("signSetGov", async () => {
    await expect(tokenManager.connect(user0).signSetGov(timelock.address, gplx.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: forbidden")

    await expect(tokenManager.connect(signer2).signSetGov(timelock.address, gplx.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")
    ...
    await tokenManager.connect(signer2).signSetGov(timelock.address, gplx.address, user1.address, 1)

    await expect(tokenManager.connect(signer2).signSetGov(timelock.address, gplx.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: already signed")

    await tokenManager.connect(signer1).signSetGov(timelock.address, gplx.address, user1.address, 1)
  })

  it("setGov", async () => {
    await gplx.setGov(gplxTimelock.address)

    await expect(tokenManager.connect(user0).setGov(gplxTimelock.address, gplx.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: forbidden")

    await expect(tokenManager.connect(wallet).setGov(gplxTimelock.address, gplx.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await tokenManager.connect(wallet).signalSetGov(gplxTimelock.address, gplx.address, user1.address)

    await expect(tokenManager.connect(wallet).setGov(user2.address, gplx.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(gplxTimelock.address, user0.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(gplxTimelock.address, gplx.address, user2.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(gplxTimelock.address, gplx.address, user1.address, 1 + 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(gplxTimelock.address, gplx.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not authorized")

    await tokenManager.connect(signer0).signSetGov(gplxTimelock.address, gplx.address, user1.address, 1)

    await expect(tokenManager.connect(wallet).setGov(gplxTimelock.address, gplx.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: insufficient authorization")

    await expect(gplxTimelock.connect(wallet).signalSetGov(gplx.address, user1.address))
      .to.be.revertedWith("GplxTimelock: forbidden")

    await tokenManager.connect(signer2).signSetGov(gplxTimelock.address, gplx.address, user1.address, 1)

    await expect(gplxTimelock.connect(wallet).setGov(gplx.address, user1.address))
      .to.be.revertedWith("GplxTimelock: action not signalled")

    await tokenManager.connect(wallet).setGov(gplxTimelock.address, gplx.address, user1.address, 1)
    ...
    expect(await gplx.gov()).eq(gplxTimelock.address)
    await gplxTimelock.connect(wallet).setGov(gplx.address, user1.address)
    expect(await gplx.gov()).eq(user1.address)
  })
})
