...
describe("Timelock", function () {
  ...
  beforeEach(async () => {
    ...
    timelock = await deployContract("Timelock", [
      ...
      gplpManager.address, // gplpManager
  ...
  it("updateUsdgSupply", async () => {
    ...
    await usdg.mint(gplpManager.address, 1000)

    expect(await usdg.balanceOf(gplpManager.address)).eq(1000)
    ...
    expect(await usdg.balanceOf(gplpManager.address)).eq(500)
    expect(await usdg.totalSupply()).eq(500)

    await timelock.updateUsdgSupply(2000)

    expect(await usdg.balanceOf(gplpManager.address)).eq(2000)
  ...
  it("initGplpManager", async () => {
    await expect(timelock.connect(user0).initGplpManager())
      .to.be.revertedWith("Timelock: forbidden")

      await gplp.setGov(timelock.address)
      await usdg.setGov(timelock.address)

      expect(await gplp.isMinter(gplpManager.address)).eq(false)
      expect(await usdg.vaults(gplpManager.address)).eq(false)
      expect(await vault.isManager(gplpManager.address)).eq(false)

      await timelock.initGplpManager()

      expect(await gplp.isMinter(gplpManager.address)).eq(true)
      expect(await usdg.vaults(gplpManager.address)).eq(true)
      expect(await vault.isManager(gplpManager.address)).eq(true)
  })
  ...
  it("initRewardRouter", async () => {
    await expect(timelock.connect(user0).initRewardRouter())
      ...
      expect(await stakedGplpTracker.isHandler(rewardRouter.address)).eq(false)
      expect(await feeGplpTracker.isHandler(rewardRouter.address)).eq(false)
      expect(await gplpManager.isHandler(rewardRouter.address)).eq(false)

      await timelock.initRewardRouter()

      expect(await stakedGplpTracker.isHandler(rewardRouter.address)).eq(true)
      expect(await feeGplpTracker.isHandler(rewardRouter.address)).eq(true)
      expect(await gplpManager.isHandler(rewardRouter.address)).eq(true)
  })
  ...
  it("processMint", async () => {
    ...
    await gplx.setGov(timelock.address)

    await expect(timelock.connect(user0).processMint(gplx.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("Timelock: forbidden")

    await expect(timelock.connect(wallet).processMint(gplx.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("Timelock: action not signalled")

    await expect(timelock.connect(user0).signalMint(gplx.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("Timelock: forbidden")

    await timelock.connect(wallet).signalMint(gplx.address, user1.address, expandDecimals(100, 18))
    ...
    await expect(timelock.connect(wallet).processMint(gplx.address, user2.address, expandDecimals(100, 18)))
      .to.be.revertedWith("Timelock: action not signalled")

    await expect(timelock.connect(wallet).processMint(gplx.address, user1.address, expandDecimals(101, 18)))
      .to.be.revertedWith("Timelock: action not signalled")

    expect(await gplx.balanceOf(timelock.address)).eq(0)
    expect(await gplx.balanceOf(user1.address)).eq(0)

    await timelock.connect(wallet).processMint(gplx.address, user1.address, expandDecimals(100, 18))
    ...
    const action1 = ethers.utils.solidityKeccak256(["string", "address", "address", "uint256"], ["mint", gplx.address, user1.address, expandDecimals(100, 18)])
  ...
  it("withdrawToken", async () => {
    ...
    await expect(timelock.connect(user0).withdrawToken(gplx.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("Timelock: forbidden")

    await expect(timelock.connect(wallet).withdrawToken(gplx.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("Timelock: action not signalled")

    await expect(timelock.connect(user0).signalWithdrawToken(gplx.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("Timelock: forbidden")

    await timelock.connect(wallet).signalWithdrawToken(gplx.address, bnb.address, user0.address, 100)
    ...
    await expect(timelock.connect(wallet).withdrawToken(gplx.address, dai.address, user0.address, 100))
      .to.be.revertedWith("Timelock: action not signalled")

    await expect(timelock.connect(wallet).withdrawToken(gplx.address, bnb.address, user1.address, 100))
      .to.be.revertedWith("Timelock: action not signalled")

    await expect(timelock.connect(wallet).withdrawToken(gplx.address, bnb.address, user0.address, 101))
      .to.be.revertedWith("Timelock: action not signalled")

    await expect(timelock.connect(wallet).withdrawToken(gplx.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("ERC20: transfer amount exceeds balance")

    await bnb.mint(gplx.address, 100)
    expect(await bnb.balanceOf(user0.address)).eq(0)
    await timelock.connect(wallet).withdrawToken(gplx.address, bnb.address, user0.address, 100)
  ...
  it("setInPrivateTransferMode", async () => {
    ...
    await expect(timelock.connect(user0).setInPrivateTransferMode(gplx.address, true))
      .to.be.revertedWith("Timelock: forbidden")

    await expect(timelock.connect(wallet).setInPrivateTransferMode(gplx.address, true))
      .to.be.revertedWith("BaseToken: forbidden")

    await gplx.setGov(timelock.address)

    expect(await gplx.inPrivateTransferMode()).eq(false)
    await timelock.connect(wallet).setInPrivateTransferMode(gplx.address, true)
    expect(await gplx.inPrivateTransferMode()).eq(true)

    await timelock.connect(wallet).setInPrivateTransferMode(gplx.address, false)
    expect(await gplx.inPrivateTransferMode()).eq(false)

    await timelock.connect(wallet).setInPrivateTransferMode(gplx.address, true)
    expect(await gplx.inPrivateTransferMode()).eq(true)

    await expect(gplx.connect(user0).transfer(user1.address, 100))
      .to.be.revertedWith("BaseToken: msg.sender not whitelisted")

    await timelock.connect(wallet).setInPrivateTransferMode(gplx.address, false)
    expect(await gplx.inPrivateTransferMode()).eq(false)

    await gplx.connect(user0).transfer(user1.address, 100)
  })
  ...
  it("setShortsTrackerAveragePriceWeight", async () => {
    await gplpManager.setGov(timelock.address)
    expect(await gplpManager.gov()).eq(timelock.address)

    await expect(timelock.connect(user0).setShortsTrackerAveragePriceWeight(1234))
      .to.be.revertedWith("Timelock: forbidden")

    expect(await gplpManager.shortsTrackerAveragePriceWeight()).eq(0)
    await timelock.setShortsTrackerAveragePriceWeight(1234)
    expect(await gplpManager.shortsTrackerAveragePriceWeight()).eq(1234)
  })

  it("setGplpCooldownDuration", async () => {
    await gplpManager.setGov(timelock.address)
    expect(await gplpManager.gov()).eq(timelock.address)

    await expect(timelock.connect(user0).setGplpCooldownDuration(3600))
      .to.be.revertedWith("Timelock: forbidden")

    await expect(timelock.connect(wallet).setGplpCooldownDuration(3 * 60 * 60))
      .to.be.revertedWith("Timelock: invalid _cooldownDuration")

    expect(await gplpManager.cooldownDuration()).eq(86400)
    await timelock.setGplpCooldownDuration(3600)
    expect(await gplpManager.cooldownDuration()).eq(3600)
  })
})
