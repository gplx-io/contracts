...
describe("GplxTimelock", function () {
  ...
  beforeEach(async () => {
    ...
    timelock = await deployContract("GplxTimelock", [
      wallet.address,
      5 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
  ...
  it("inits", async () => {
    ...
    await expect(deployContract("GplxTimelock", [
      wallet.address,
      7 * 24 * 60 * 60 + 1,
      7 * 24 + 60 * 60,
      rewardManager.address,
      ...
      mintReceiver.address,
      1000
    ])).to.be.revertedWith("GplxTimelock: invalid _buffer")
    ...
    await expect(deployContract("GplxTimelock", [
      ...
      mintReceiver.address,
      1000
    ])).to.be.revertedWith("GplxTimelock: invalid _longBuffer")
  })
  ...
  it("setTokenConfig", async () => {
    ...
    await expect(timelock.connect(user0).setTokenConfig(
      ...
      0,
      0
    )).to.be.revertedWith("GplxTimelock: forbidden")
    ...
    await expect(timelock.connect(wallet).setTokenConfig(
      ...
      0,
      0
    )).to.be.revertedWith("GplxTimelock: token not yet whitelisted")
  ...
  it("setBuffer", async () => {
    const timelock0 = await deployContract("GplxTimelock", [
      user1.address,
      3 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
      rewardManager.address,
    ...
    await expect(timelock0.connect(user1).setBuffer(3 * 24 * 60 * 60 - 10))
      .to.be.revertedWith("GplxTimelock: buffer cannot be decreased")
  ...
  it("setMaxLeverage", async () => {
    ...
    await expect(timelock.connect(wallet).setMaxLeverage(vault.address, 49 * 10000))
      .to.be.revertedWith("GplxTimelock: invalid _maxLeverage")
  ...
  it("approve", async () => {
    ...
    await expect(timelock.connect(wallet).approve(dai.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("GplxTimelock: action not signalled")
    ...
    await expect(timelock.connect(wallet).approve(dai.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("GplxTimelock: action time not yet passed")
    ...
    await expect(timelock.connect(wallet).cancelAction(action0))
      .to.be.revertedWith("GplxTimelock: invalid _action")
  ...
  it("addExcludedToken", async () => {
    const gplx = await deployContract("GPLX", [])
    await expect(timelock.connect(user0).addExcludedToken(gplx.address))
      .to.be.revertedWith("GplxTimelock: forbidden")

    expect(await timelock.excludedTokens(gplx.address)).eq(false)
    await timelock.connect(wallet).addExcludedToken(gplx.address)
    expect(await timelock.excludedTokens(gplx.address)).eq(true)
  })
  ...
  it("setInPrivateTransferMode", async () => {
    ...
    await timelock.addExcludedToken(gplx.address)
    await expect(timelock.connect(wallet).setInPrivateTransferMode(gplx.address, true))
      .to.be.revertedWith("GplxTimelock: invalid _inPrivateTransferMode")
  ...
  it("setExternalAdmin", async () => {
    ...
    await expect(timelock.connect(wallet).setExternalAdmin(timelock.address, user3.address))
      .to.be.revertedWith("GplxTimelock: invalid _target")
  })
