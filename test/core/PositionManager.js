  ...
  beforeEach(async () => {
    ...
    deployTimelock = async () => {
      return await deployContract("Timelock", [
        ...
        ethers.constants.AddressZero, // _gplpManager
...
describe("PositionManager next short data calculations", function () {
  ...
  async function debugState(label = "") {
    ...
    const aum = await gplpManager.getAum(true)
  ...
  it("PositionManager and GplpManager init with shortsTracker", async () => {
    const [
      positionManagerShortTracker,
      gplpManagerShortTracker,
      avgeragePrice,
      size
    ] = await Promise.all([
      positionManager.shortsTracker(),
      gplpManager.shortsTracker(),
    ...
    expect(gplpManagerShortTracker, 'gplpManager shortsTracker').eq(shortsTracker.address)
  ...
  it("updates global short average prices on position decreases", async () => {
    await shortsTracker.setIsGlobalShortDataReady(true)
    await gplpManager.setShortsTrackerAveragePriceWeight(10000)
    expect(await gplpManager.shortsTrackerAveragePriceWeight()).to.be.equal(10000)
    ...
    let aumBefore = await gplpManager.getAum(true)
    ...
    let aumAfter = await gplpManager.getAum(true)
    ...
    aumBefore = await gplpManager.getAum(true)
    ...
    aumAfter = await gplpManager.getAum(true)
